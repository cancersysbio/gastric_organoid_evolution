...
bc_rg_file = 'BC_RG_AssociatedUMICount.txt'
bc_adt_file = 'Hash_correlation.txt'

## Output files names ##
outfile = 'Cell_Metadata_not_updated.txt'
outfile_min10 = 'Cell_Metadata_min10cells_perECBrg_not_updated.txt'
outfile_full = 'Cell_Metadata_full_not_updated.txt'
outfile_summary = 'Cell_Metadata_summary_not_updated.txt'
outfile_ecbhash = 'Cell_Metadata_ecbs_per_hashSample_not_updated.txt'
...
with open(os.path.join(ecb_dir, bc_rg_file), 'rt') as efile:
    efile.readline()
    for line in efile:
        bc, updated_rg, umi_count, read_count  = line.strip().split('\t')
        # RG cell count
