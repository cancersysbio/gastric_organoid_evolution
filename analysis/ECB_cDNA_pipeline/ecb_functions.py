...
## Function: extract 10x barcode (bc), UMI, and extracellular barcode (ecb) ##
...
def extract_fastQ(fq_dir, fastqname, ecbhandle, hamming_distance):
            ...
            if counter % 4 == 1: # line 2, line 6, line 10, ... 
                ...
                if len(line1) >= 36: # 36 is the length of bc, umi, and polyT.
                    ...
                    if polyT_ham <= hamming_distance: # if quality is good enough
                        bc_umi = line1[:26]
                        if len(line2) >= 50:
                            ecb_handle_ham = hamming1(line2[0:20], ecbhandle) # test ecb pcr handle quality
                            if ecb_handle_ham <= hamming_distance:
                                ecb = line2[20:50]
                                if readID1 == readID2:
                                    if bc_umi not in R1R2_dict:
                                        R1R2_dict[bc_umi] = {ecb:1} # store bc_UMI and ecb
                                    else:
                                        temp_dict = R1R2_dict[bc_umi] 
                                        if ecb not in temp_dict:
                                            temp_dict[ecb] = 1
                                        else:
                                            temp_dict[ecb] += 1 # count how many when more than 1
